...
describe('The OptionDropdown Component', () => {
  ...
  it('can handle on option being clicked', () => {
    ...
    rendered.find('.history-dropdown-trigger div').at(0).simulate('click');

    expect(rendered).to.be.ok;
    rendered.find('li').at(0).simulate('click');
    expect(clicked).to.equal('derp');

    rendered.find('li').at(1).simulate('click');
    expect(clicked).to.equal('herp');

    rendered.find('li').at(2).simulate('click');
